...
/// Returns `true` if `ty` is valid immutable runtime payload data.
///
/// Immutable contract fields are serialized into deployed runtime bytecode, so their type must be
/// fully known, self-contained runtime data with a concrete non-zero fixed-size memory layout.
pub fn is_runtime_immutable_payload_ty(db: &dyn HirAnalysisDb, ty: TyId<'_>) -> bool {
    is_runtime_immutable_payload_ty_in(db, &EVM_LAYOUT, ty)
}

pub fn is_runtime_immutable_payload_ty_in(
    db: &dyn HirAnalysisDb,
    layout: &TargetDataLayout,
    ty: TyId<'_>,
) -> bool {
    fn inner<'db>(
        ...
        if let Some(normalized) = normalize_ty_for_layout(db, ty) {
            return inner(db, layout, normalized, visiting);
        }
        ...
        let supported = if ty.is_tuple(db) {
            ty.field_types(db)
                .into_iter()
                .all(|field_ty| inner(db, layout, field_ty, visiting))
        } else if ty.is_array(db) {
            array_len(db, ty).is_some()
                && array_elem_ty(db, ty).is_some_and(|elem_ty| inner(db, layout, elem_ty, visiting))
            ...
            ty.base_ty(db).data(db)
        {
            ...
            match adt_def.adt_ref(db) {
                AdtRef::Struct(_) => ty
                    ...
                    .into_iter()
                    .all(|field_ty| inner(db, layout, field_ty, visiting)),
                AdtRef::Enum(enm) => enm.variants(db).into_iter().all(|variant| {
                    let ctor = ConstructorKind::Variant(EnumVariant::new(enm, variant.idx), ty);
                    ctor.field_types(db)
                        .into_iter()
                        .all(|field_ty| inner(db, layout, field_ty, visiting))
                }),
        ...
        visiting.remove(&ty);
        supported && ty_memory_size_in(db, layout, ty).is_some_and(|size| size > 0)
    }

    inner(db, layout, ty, &mut FxHashSet::default())
}

/// Returns the canonical immutable payload size for `ty`.
///
/// This is the size written into the deployed runtime bytecode tail for an immutable field.
pub fn immutable_payload_size(db: &dyn HirAnalysisDb, ty: TyId<'_>) -> Option<usize> {
    immutable_payload_size_in(db, &EVM_LAYOUT, ty)
}

pub fn immutable_payload_size_in(
    db: &dyn HirAnalysisDb,
    layout: &TargetDataLayout,
    ty: TyId<'_>,
) -> Option<usize> {
    is_runtime_immutable_payload_ty_in(db, layout, ty).then(|| ty_memory_size_in(db, layout, ty))?
}
