...
impl<'db> Borrowck<'db> {
    ...
    pub fn shape(&self, ty: TyId<'db>) -> Result<ShapeId<'db>, SemanticBorrowDiagnostic<'db>> {
        capability_shape(
            self.db,
            self.instance
                .key(self.db)
    ...
    pub fn resolve_place(&self, state: &BorrowState<'db>, place: &NPlace<'db>) -> Resolution<'db> {
        let parents = match place.base {
            NPlaceBase::Root(_) => Vec::new(),
            NPlaceBase::CapabilityTarget { carrier } => {
                self.resolve_capability(state.value(carrier)).parents
            }
        };
        Resolution {
    ...
    pub fn write_region(
        ...
        state
            .write_region(&mut self.inventory.values, region, value)
            .map_err(|error| {
                self.internal_diag(origin, format!("unresolved capability write: {error:?}"))
            })
    }
    ...
    pub fn solve(&mut self) -> Result<(), SemanticBorrowDiagnostic<'db>> {
        self.prepare_calls()?;
    ...
    fn transfer(
        ...
        let NStatementKind::Define { result, expr } = &statement.kind else {
            ...
            return self.write_region(state, &region, &replacement, statement.origin);
        };
        ...
        let mut value = match expr {
            ...
            NExpr::Load { place, mode } => {
                ...
                if *mode == ReadMode::Move && place.ty.as_capability(self.db).is_none() {
                    ...
                    self.write_region(state, &region, &empty, statement.origin)?;
                }
                value
            }
            NExpr::Borrow { place, .. } => {
