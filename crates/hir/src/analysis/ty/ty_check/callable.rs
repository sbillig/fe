...
use crate::analysis::{
    ...
    ty::{
        ...
        trait_resolution::{TraitSolveCx, constraint::collect_func_decl_constraints},
        ty_def::{BorrowKind, CapabilityKind},
        ty_def::{InvalidCause, TyBase, TyData, TyFlags, TyId},
        ty_error::explicit_value_ty_wf_diag,
    ...
    pub(super) fn check_args(
        ...
        for (i, (given, expected)) in args.into_iter().zip(expected_arg_tys.iter()).enumerate() {
            ...
            if let Some(inst) = self.trait_inst {
                ...
                expected = tc.normalize_trait_inst_ty_in_scope(
                    expected,
                    inst,
                    tc.env.scope(),
                    tc.env.assumptions(),
            ...
            tc.env.mark_explicit_value_wf_checked(given.expr);
            if let Some(diag) = explicit_value_ty_wf_diag(
                db,
                TraitSolveCx::new(db, tc.env.scope()).with_assumptions(tc.env.assumptions()),
                expected,
            ...
            let variant_ctor_tyvar_arg =
                matches!(self.callable_def, CallableDef::VariantCtor(_)) && expected.is_ty_var(db);
            ...
            } else if variant_ctor_tyvar_arg {
                tc.coerce_inferred_expr_capability(given.expr, &given.expr_prop)
            } else {
...
impl<'db> Callable<'db> {
    pub(super) fn enqueue_constraints(
        ...
        let db = tc.db;
        let include_parent = !self
            .callable_def
            .scope()
            .parent_item(db)
            .is_some_and(|parent| {
                self.trait_inst.is_some() && matches!(parent, ItemKind::Trait(_))
            });
        let constraints = collect_func_decl_constraints(db, self.callable_def, include_parent);
        let instantiated = constraints.instantiate(db, &self.generic_args);
        let duplicate_impl_constraints = self.trait_inst.is_none()
            && matches!(
                self.callable_def.scope().parent_item(db),
                Some(ItemKind::Impl(_))
            );
