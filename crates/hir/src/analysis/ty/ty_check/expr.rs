...
use crate::analysis::{
    ...
    name_resolution::{
        ...
        method_selection::{
            MethodCandidate, MethodSelectionError, select_method_candidate_with_solve_cx,
        },
...
impl<'db> TyChecker<'db> {
    ...
    fn check_cast(
        ...
        let mut from = self.normalize_ty(inner_prop.ty);
        let to = self.normalize_ty(target_ty);
    ...
    fn check_call(&mut self, expr: ExprId, expr_data: &Expr<'db>) -> ExprProp<'db> {
        ...
        let normalized_ret_ty = if let Some(inst) = callable.trait_inst() {
    ...
    fn inferred_provider_ty_for_effect_arg(
        ...
        let provider_ty_for_binding = |binding: LocalBinding<'db>| match binding {
            LocalBinding::EffectParam { site, idx, .. } => Some(
                self.env
                    .effect_param_provider_ty(site, idx)
                    .unwrap_or(provider.ty),
            ),
            LocalBinding::Param {
                ...
                let contract = match effect_site {
                    ...
                    EffectParamSite::Func(_) => {
                        unreachable!("effect field bindings cannot originate from function sites")
                    }
                };
            ...
            LocalBinding::Local { .. } | LocalBinding::Param { .. } => Some(provider.ty),
        };
        ...
        match pass_mode {
            super::EffectPassMode::ByValue => {
                let binding = match arg {
                    ...
                    super::EffectArg::Place(_) => None,
                };
                Some(
                    self.table.fold_ty(
                        self.db,
                        binding
                            .and_then(provider_ty_for_binding)
                            .unwrap_or(provider.ty),
                    ),
                )
            }
            ...
            super::EffectPassMode::ByPlace => {
                ...
                Some(self.table.fold_ty(
                    self.db,
                    provider_ty_for_binding(binding).unwrap_or(provider.ty),
                ))
            }
    ...
    fn renormalize_effect_provider_target_ty_in_scope(
        ...
        self.normalize_ty_in_scope(target_ty, scope, assumptions)
    ...
    fn effect_provider_target_resolution_in_scope(
        ...
        let resolution = (|| {
            ...
            let mut target_ty = self
                .normalize_ty_in_scope(target_assoc, scope, assumptions)
                .fold_with(self.db, &mut self.table);
    ...
    fn check_method_call(&mut self, expr: ExprId, expr_data: &Expr<'db>) -> ExprProp<'db> {
        ...
        let mut candidate = select_method_candidate_with_solve_cx(
            self.db,
        ...
        if matches!(candidate, Err(MethodSelectionError::NotFound)) {
            for &receiver_ty in receiver_tys.iter().skip(1) {
                ...
                let fallback = select_method_candidate_with_solve_cx(
                    self.db,
        ...
        let candidate = match candidate {
            ...
            Err(err) => match err {
                ...
                MethodSelectionError::UnsatisfiedTraitMethod(unsatisfied) => {
                    self.push_diag(TyDiagCollection::from(
                        TraitConstraintDiag::TraitBoundNotSat {
                            span: call_span.clone().into(),
                            primary_goal: unsatisfied.goal,
                            unsat_subgoal: unsatisfied.unsat_subgoal,
                            required_by: None,
                        },
                    ));
                    return ExprProp::invalid(self.db);
    ...
    fn check_record_init_fields(&mut self, record_like: &RecordLike<'db>, expr: ExprId) {
        ...
        for (i, field) in fields.iter().enumerate() {
            ...
            let prop = if expected.is_ty_var(rec_checker.tc.db) {
                let fresh = rec_checker.tc.fresh_ty();
                let prop = rec_checker.tc.check_expr(field.expr, fresh);
                let actual = rec_checker
                    .tc
                    .coerce_inferred_expr_capability(field.expr, &prop);
                let ty = rec_checker.tc.unify_ty(
                    Typeable::Expr(field.expr, prop.clone()),
                    actual,
                    expected,
                );
                ExprProp { ty, ..prop }
    ...
    fn check_tuple(
        ...
        let mut elem_tys = match expected.decompose_ty_app(self.db) {
            (base, args) if base.is_tuple(self.db) && args.len() == elems.len() => args.to_vec(),
            _ => self.fresh_tys_n(elems.len()),
        };

        for (elem, elem_ty) in elems.iter().zip(elem_tys.iter_mut()) {
            let prop = self.check_expr(*elem, *elem_ty);
            if elem_ty.is_ty_var(self.db) {
                *elem_ty = self.coerce_inferred_expr_capability(*elem, &prop);
            } else {
                *elem_ty = prop.ty;
            }
    ...
    fn check_array(
        ...
        for elem in elems {
            ...
            if expected_elem_ty.is_ty_var(self.db) {
                expected_elem_ty = self.coerce_inferred_expr_capability(*elem, &prop);
            } else {
    ...
    fn check_if(
        ...
        let ty = match else_ {
            Some(else_) => {
                ...
                let then_expected = self.branch_expected(expected);
                let then_ty = self.check_expr(*then, then_expected).ty;
                self.env.leave_scope();
                self.env.clear_pending_bindings();
                self.env.leave_scope();
                let else_expected = self.branch_expected(expected);
                let else_ty = self.check_expr_in_new_scope(*else_, else_expected).ty;
                self.join_branch_tys(expr.span(self.body()).into(), then_ty, else_ty)
            }
            ...
            None => {
                ...
                self.env.enter_scope(*then);
                self.env.flush_pending_bindings();
                let then_expected = self.fresh_ty();
    ...
    fn check_match(
        ...
        for arm in arms.iter() {
            ...
            self.env.flush_pending_bindings();
            let arm_expected = self.branch_expected(expected);
            let arm_ty = self.check_expr(arm.body, arm_expected).ty;
            self.env.leave_scope();
            match_ty = Some(match match_ty {
                Some(match_ty) => {
                    self.join_branch_tys(expr.span(self.body()).into(), match_ty, arm_ty)
                }
                None => arm_ty,
            });
        }
        ...
        ExprProp::new(match_ty.unwrap_or_else(|| TyId::never(self.db)), true)
    }

    fn branch_expected(&mut self, expected: TyId<'db>) -> TyId<'db> {
        let expected = self.normalize_ty(expected);
        if expected.base_ty(self.db).is_ty_var(self.db) {
            self.fresh_ty()
        } else {
            expected
        }
    ...
    fn join_branch_tys(
        ...
        if lhs.is_never(self.db) {
            return rhs;
        }
        if rhs.is_never(self.db) {
            return lhs;
        }

        self.equate_ty(lhs, rhs, span)
    }
    ...
    fn check_ops_trait(
        ...
        let mut method_candidate = select_method_candidate_with_solve_cx(
            self.db,
            c_lhs_ty.value,
        ...
        let ret_ty = if let Some(inst) = callable.trait_inst() {
            ...
            self.normalize_ty(callable.ret_ty(self.db))
        };
        self.env.register_callable(expr, callable);
