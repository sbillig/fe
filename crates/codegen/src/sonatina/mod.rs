...
struct LowerCtx<'a, 'db, C: sonatina_ir::func_cursor::FuncCursor> {
    fb: &'a mut sonatina_ir::builder::FunctionBuilder<C>,
    db: &'db DriverDataBase,
    ...
    runtime_param_masks: &'a FxHashMap<String, Vec<bool>>,
        ...
        Rvalue::Call(call) => {
            ...
            if let Some(mask) = ctx.runtime_param_masks.get(callee_name) {
                ...
                for &effect_arg in &call.effect_args {
                    ...
                    let val = lower_value(ctx, effect_arg)?;
                    args.push(val);
                }
            }
...
fn lower_value<'db, C: sonatina_ir::func_cursor::FuncCursor>(
    ...
    let value_data = &ctx.body.values[value_id.index()];
