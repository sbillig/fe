...
use sonatina_codegen::object::{PcMapEntry, SectionObservability, UnmappedReason};
...
pub fn emit_bytecode_instruction_facts(
    ...
    .expect("bytecode emission without observability cannot fail attribution checks")
}
...
fn emit_evm_bytecode_instruction_facts_with_observability(
    ...
    if let Some(observability) = observability {
        crate::sonatina::observability::verify_received_observability(
            owner_key,
            bytecode,
            observability,
        )?;
    ...
    while pc < code_len {
            ...
            pc_map_entry.filter(|entry| entry.attribution.unmapped_reason().is_none())
        {
            ...
            if let Some(raw) = entry.attribution.post_opt_provenance() {
                let frontend_origin = serde_json::from_str::<OriginExportKey>(raw).map_err(|err| {
                    crate::LowerError::Internal(format!(
                        "mapped bytecode attribution for `{owner_key}` has undecodable post-opt provenance: {err}"
                    ))
                })?;
                if frontend_origin.kind() != SONATINA_POSTOPT_INST_KIND {
                    return Err(crate::LowerError::Internal(format!(
                        "mapped bytecode attribution for `{owner_key}` has unexpected provenance kind `{}`",
                        frontend_origin.kind()
                    )));
                }
                let prepared_inst = prepared_inst.ok_or_else(|| {
                    crate::LowerError::Internal(format!(
                        "mapped bytecode attribution for `{owner_key}` has no prepared instruction"
                    ))
                })?;
                let known_sonatina_endpoint_nodes = known_sonatina_endpoint_nodes.ok_or_else(|| {
                    crate::LowerError::Internal(format!(
                        "mapped bytecode attribution for `{owner_key}` cannot verify its post-opt endpoint"
                    ))
                })?;
                if !known_sonatina_endpoint_nodes.contains(&frontend_origin) {
                    return Err(crate::LowerError::Internal(format!(
                        "mapped bytecode attribution for `{owner_key}` references absent post-opt endpoint `{}`",
                        frontend_origin.canonical_storage_key()
                    )));
                }
            ...
            for inst in module.trace_instructions(function_ref, block) {
                ...
                match sonatina_frontend_origin_for_inst(module, function_ref, inst) {
                    ...
                    FrontendOriginLookup::Undecodable(raw) => {
                        return Err(crate::LowerError::Internal(format!(
                            "undecodable frontend origin for `{owner_key}`: `{raw}`"
                        )));
                    }
...
mod tests {
    ...
    use sonatina_codegen::object::{SectionObservability, UnmappedReason};
    use trace_facts::{
    ...
    fn valid_unmapped_observability(section_bytes: u32) -> SectionObservability {
        let mut coverage = sonatina_codegen::object::UnmappedReasonCoverage::default();
        coverage.add_bytes(UnmappedReason::Unknown, section_bytes);
        SectionObservability {
            schema_version: sonatina_codegen::object::OBSERVABILITY_SCHEMA_VERSION,
            section: "runtime".into(),
            section_bytes,
            code_bytes: section_bytes,
            data_bytes: 0,
            embed_bytes: 0,
            mapped_code_bytes: 0,
            unmapped_code_bytes: section_bytes,
            unmapped_reason_coverage: coverage,
            pc_map: Vec::new(),
        }
    }

    #[test]
    fn public_section_trace_api_rejects_corrupt_observability() {
        let mut corrupt_aggregate = valid_unmapped_observability(1);
        corrupt_aggregate.mapped_code_bytes = 1;
        assert!(
            emit_bytecode_instruction_facts_with_observability(
                "contract:Fib",
                "runtime",
                &[0x00],
                None,
                Some(&corrupt_aggregate),
                None,
            )
            .unwrap_err()
            .to_string()
            .contains("mapped 1 + unmapped 1 != code 1")
        );

        let mut missing_pc_map = valid_unmapped_observability(1);
        missing_pc_map.mapped_code_bytes = 1;
        missing_pc_map.unmapped_code_bytes = 0;
        missing_pc_map.unmapped_reason_coverage = Default::default();
        assert!(
            emit_bytecode_instruction_facts_with_observability(
                "contract:Fib",
                "runtime",
            ...
            .unwrap_err()
            .to_string()
            .contains("pc map derives 0 mapped bytes")
        );
    }

    #[test]
    fn public_observed_map_trace_api_rejects_corrupt_observability() {
        let mut corrupt_aggregate = valid_unmapped_observability(1);
        corrupt_aggregate.unmapped_reason_coverage.unknown = 0;
        let mut missing_pc_map = valid_unmapped_observability(1);
        missing_pc_map.mapped_code_bytes = 1;
        missing_pc_map.unmapped_code_bytes = 0;
        missing_pc_map.unmapped_reason_coverage = Default::default();

        for (observability, expected) in [
            (corrupt_aggregate, "unmapped-reason total"),
            (missing_pc_map, "pc map derives 0 mapped bytes"),
        ] {
            let bytecode = std::collections::BTreeMap::from([(
                ...
                SonatinaContractBytecode {
                    deploy: Vec::new(),
                    runtime: vec![0x00],
                    deploy_observability: None,
                    runtime_observability: Some(observability),
                },
            )]);

            let err = emit_observed_bytecode_trace_facts(
                "demo",
                "demo",
                "function:runtime",
    ...
    fn observed_bytecode_trace_emits_creation_and_runtime_code_objects() {
        ...
        let creation_observability = SectionObservability {
            ...
            unmapped_reason_coverage: {
                let mut coverage = sonatina_codegen::object::UnmappedReasonCoverage::default();
                coverage.add_bytes(UnmappedReason::Unknown, 2);
                coverage
            },
            pc_map: Vec::new(),
        };
    ...
    fn bytecode_observability_applies_pc_map_ranges_to_each_instruction_start() {
        ...
        let vcode = evm_vcode_inst_key(sonatina_owner, func, VCodeInst(0));
        let known = std::collections::BTreeSet::from([postopt.clone()]);
    ...
    fn bytecode_observability_does_not_invent_postopt_lineage_without_known_trace_nodes() {
        ...
        let err = emit_bytecode_instruction_facts_with_observability(
            "contract:Fib",
            "runtime",
            &[0x5f],
            Some(sonatina_owner),
        ...
        assert!(
            err.to_string()
                .contains("cannot verify its post-opt endpoint")
        );
    }
    ...
    fn bytecode_observability_refuses_index_keyed_provenance() {
        ...
        assert!(
            err.to_string()
                .contains("references absent post-opt endpoint")
        );
    }
    ...
    fn bytecode_observability_missing_provenance_entry_produces_no_join() {
        ...
        let observability = SectionObservability {
            ...
            unmapped_reason_coverage: {
                let mut coverage = sonatina_codegen::object::UnmappedReasonCoverage::default();
                coverage.add_bytes(UnmappedReason::MissingProvenance, 1);
                coverage
            },
            pc_map: vec![PcMapEntry {
                pc_start: 0,
    ...
    #[test]
    fn undecodable_post_opt_provenance_is_rejected() {
        use sonatina_codegen::{
            machinst::vcode::VCodeInst,
            object::{OBSERVABILITY_SCHEMA_VERSION, PcMapEntry, SectionObservability},
        };
        ...
        // Provenance present but not decodable as an OriginExportKey is schema
        // drift and must prevent trace publication.
        let observability = SectionObservability {
            schema_version: OBSERVABILITY_SCHEMA_VERSION,
            section: "runtime".into(),
            section_bytes: 1,
        ...
        .unwrap_err();
        assert!(err.to_string().contains("undecodable post-opt provenance"));
    }

    #[test]
    fn unknown_postopt_endpoint_is_rejected() {
        use sonatina_codegen::{
            machinst::vcode::VCodeInst,
            object::{OBSERVABILITY_SCHEMA_VERSION, PcMapEntry, SectionObservability},
        };
        ...
        // A well-formed postopt endpoint absent from the known-node set must
        // prevent trace publication.
        let observability = SectionObservability {
            schema_version: OBSERVABILITY_SCHEMA_VERSION,
            section: "runtime".into(),
            section_bytes: 1,
    ...
    #[test]
    fn post_opt_provenance_with_foreign_kind_is_rejected() {
        use sonatina_codegen::{
            machinst::vcode::VCodeInst,
            object::{OBSERVABILITY_SCHEMA_VERSION, PcMapEntry, SectionObservability},
        };
        ...
        .unwrap_err();
        assert!(err.to_string().contains("unexpected provenance kind"));
    }
    ...
    #[test]
    fn sonatina_trace_view_adapter_rejects_undecodable_frontend_origin() {
        use std::sync::Arc;
        ...
        builder
            .func
            .set_inst_frontend_origin(inst, Arc::from("not json"));
        builder.insert_return(value);
        builder.seal_all();
        builder.finish();
        let module = mb.build();
        ...
        let err =
            emit_sonatina_trace_view_facts("owner:test", &module, CompilerPhase::SonatinaPreOpt)
                .unwrap_err();
        assert!(err.to_string().contains("undecodable frontend origin"));
    }
