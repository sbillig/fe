...
fn emit_observable_package_trace_facts(
    ...
    for (contract_name, artifact) in &bytecode {
        if let Some(obs) = artifact.runtime_observability.as_ref() {
            verify_received_observability(
                &format!("{contract_name}:runtime"),
                &artifact.runtime,
                obs,
            )?;
        }
...
fn checked_unmapped_reason_total(
    ...
    [
        coverage.missing_provenance,
        coverage.no_machine_inst,
        coverage.label_or_fixup_only,
        coverage.synthetic,
        coverage.unknown,
    ]
    .into_iter()
