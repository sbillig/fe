...
use hir::{
    ...
    hir_def::{
        Contract, FieldDefListId, Func, IdentId, ItemKind, Mod, PathId, Struct, TopLevelMod, Trait,
        scope_graph::ScopeId,
    },
};
use serde::Serialize;
...
fn recv_arm_to_abi_entry(
    ...
    let (variant_struct, variant_ty) = resolve_recv_arm_variant_for_abi(db, arm_view)?;
    ...
    if !variant_has_canonical_json_abi_shape(db, variant_struct) {
        if !has_manual_msg_variant_impl_for_abi(db, arm_view.contract(db), variant_ty, sol_ty) {
            return Err(format!(
                "recv arm type `{}` does not implement `MsgVariant<{}>`",
                variant_ty.pretty_print(db),
                sol_ty.pretty_print(db)
            ));
        }
...
fn resolve_recv_msg_mod_for_abi<'db>(
    db: &'db DriverDataBase,
    contract: Contract<'db>,
    msg_path: PathId<'db>,
) -> Result<Mod<'db>, String> {
    let assumptions = PredicateListId::empty_list(db);
    match resolve_path(db, msg_path, contract.scope(), assumptions, false) {
        Ok(PathRes::Mod(scope)) => match scope {
            ScopeId::Item(ItemKind::Mod(mod_)) => Ok(mod_),
            _ => unreachable!(),
        },
        Ok(PathRes::Ty(ty) | PathRes::TyAlias(_, ty)) => Err(format!(
            "recv root `{}` resolved to type `{}` instead of a module",
            msg_path.pretty_print(db),
            ty.pretty_print(db)
        )),
        Ok(other) => Err(format!(
            "recv root `{}` resolved to {} instead of a module",
            msg_path.pretty_print(db),
            other.kind_name()
        )),
        Err(err) => Err(format!(
            "failed to resolve recv root `{}` during ABI generation: {err:?}",
            msg_path.pretty_print(db)
        )),
    }
}

fn resolve_recv_arm_variant_for_abi<'db>(
    db: &'db DriverDataBase,
    arm_view: hir::semantic::RecvArmView<'db>,
) -> Result<(Struct<'db>, TyId<'db>), String> {
    let Some(arm) = arm_view.arm(db) else {
        return Err("missing recv arm during ABI generation".to_string());
    };
    let Some(variant_path) = arm.variant_path(db) else {
        return Err("missing recv arm variant path during ABI generation".to_string());
    };

    let recv = arm_view.recv(db);
    let contract = recv.contract(db);
    let assumptions = PredicateListId::empty_list(db);
    let scope = match recv.msg_path(db) {
        Some(msg_path) => resolve_recv_msg_mod_for_abi(db, contract, msg_path)?.scope(),
        None => contract.scope(),
    };
    let variant_ty = match resolve_path(db, variant_path, scope, assumptions, false) {
        Ok(PathRes::Ty(ty) | PathRes::TyAlias(_, ty)) => ty,
        Ok(other) => {
            return Err(format!(
                "recv arm `{}` resolved to {} instead of a type",
                variant_path.pretty_print(db),
                other.kind_name()
            ));
        }
        Err(err) => {
            return Err(format!(
                "failed to resolve recv arm `{}` during ABI generation: {err:?}",
                variant_path.pretty_print(db)
            ));
        }
    };
    ...
    Ok((variant_struct, variant_ty))
}

fn has_manual_msg_variant_impl_for_abi<'db>(
    db: &'db DriverDataBase,
    contract: Contract<'db>,
    variant_ty: TyId<'db>,
    abi_ty: TyId<'db>,
    ...
    contract
        ...
        .iter()
        .any(|impl_trait| {
            impl_trait.ty(db) == variant_ty
                && impl_trait.trait_inst(db).is_some_and(|inst| {
                    inst.def(db) == msg_variant_trait && inst.args(db).get(1) == Some(&abi_ty)
                })
        })
}
